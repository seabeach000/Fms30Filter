...
CFMS30InputPin::CFMS30InputPin(TCHAR *pName, CFMS30Splitter *pFilter, CCritSec *pLock, HRESULT *phr)
	:CBasePin(pName,(CBaseFilter*)pFilter,pLock,phr,L"Input",PINDIR_INPUT)
{
}


CFMS30InputPin::~CFMS30InputPin()
{
	if (m_pAVIOContext) {
...
HRESULT CFMS30InputPin::GetAVIOContext(AVIOContext** ppContext)
{
...
STDMETHODIMP CFMS30InputPin::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
...
HRESULT CFMS30InputPin::CheckMediaType(const CMediaType* pmt)
{
	return S_OK;
}

HRESULT CFMS30InputPin::CheckConnect(IPin* pPin)
{
	HRESULT hr;
...
HRESULT CFMS30InputPin::BreakConnect()
{
	HRESULT hr;
	...
	if (FAILED(hr = (static_cast<CFMS30Splitter *>(m_pFilter))->BreakInputConnection())) {
		return hr;
	}
...
HRESULT CFMS30InputPin::CompleteConnect(IPin* pPin)
{
	HRESULT hr;
	...
	if (FAILED(hr = (static_cast<CFMS30Splitter *>(m_pFilter))->CompleteInputConnection())) {
		return hr;
	}
...
STDMETHODIMP CFMS30InputPin::BeginFlush()
{
	return E_UNEXPECTED;
}

STDMETHODIMP CFMS30InputPin::EndFlush()
{
	return E_UNEXPECTED;
}

int CFMS30InputPin::Read(void *opaque, uint8_t *buf, int buf_size)
{
	CFMS30InputPin *pin = static_cast<CFMS30InputPin *>(opaque);
	CAutoLock lock(pin);
...
int64_t CFMS30InputPin::Seek(void *opaque, int64_t offset, int whence)
{
	CFMS30InputPin *pin = static_cast<CFMS30InputPin *>(opaque);
	CAutoLock lock(pin);
